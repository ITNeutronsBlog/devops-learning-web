...
def execute_failover(config, state):
    ...
    try:
        ...
        try:
            ...
            # Sanitize postgresql.auto.conf after promotion
            log.info("  🧹 Sanitizing postgresql.auto.conf...")
            timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
            clean_autoconf = (
                "sudo tee /var/lib/postgresql/data/postgresql.auto.conf > /dev/null << 'AUTOCONF_EOF'\n"
