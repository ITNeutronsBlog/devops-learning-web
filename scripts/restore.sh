...
docker stop devops-learning-db 2>/dev/null || true
...
docker start devops-learning-db
...
docker exec devops-learning-db psql -U devops -d devops_learning -c "SELECT COUNT(*) as video_count FROM videos;"
